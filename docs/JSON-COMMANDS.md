...
**Returns:** The index value or -1 if not found of out of range (int)
...
### `Object Length`
Number of keys in an object specified in `path` (int)
...
### `String Append`
It appends a `value` string to the existing in `path`
...
TBD
### `Delete`
It deletes data stored in `path`. If `key` or `path` doesn't exist, this command doesn't throw an exception. 
...
### `Get`
It returns the value stored in `paths`. More than one path can be specified and this command. This command doesn't 
provide some of the features includes in [JSON.GET command in ReJson](https://oss.redislabs.com/redisjson/commands/#jsonget), 
...
want to get values as JSON string you can use `RedisJsonClient::executeRawCommand('JSON.GET', ...$params)` where you 
can include the `$params` as specified in the link above 
...
It increments/multiplies by a `number` (int or float) the value stored in `path`
...
**Exceptions:** `ResponseException` if `key` or `path` doesn't exist, we try to increment/multiply by which cannot be 
cast to int or float or we try to increment/multiply no int|float values stored in `path`

### `Memory Usage`
It the memory used for the value stored in `path`
...
**Returns:** Array where the first element contains the data of `path` for the first key provided, the second element 
contains the data for the second `key` provided and so on. 
